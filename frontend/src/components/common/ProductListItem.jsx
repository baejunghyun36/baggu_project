...
const ItemImg = styled.div`
  ${props =>
    ...
    `}
  ${tw`w-[100px] min-w-[100px] h-[100px] rounded border bg-cover bg-center border-1`}
`;
...
// Main Container
function ProductListItem({ item, onClick, selected, checkShow }) {
  /*
  < props >
  1. item : item 데이터
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 w-full justify-between">
        <ItemInfo
          chipState={item.state === 1 || item.state === 2}
          id="itemInfo"
        >
          <p>{item.title}</p>
          <span>
